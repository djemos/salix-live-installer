        ...
        def partition_list_initialization() :
            ...
            for drive in disk_device:
                ...
                for line in parted_output:
                    ...
                    #elif line.startswith('Disk') and line.find('Flags') == -1:
                    elif line.startswith('Disk') and 'Flags' not in line:
...
\nYou cannot proceed any further! """))
    ...
    def salix_install_process(self):
        ...
        if liveclone_install == True : # We are in a LiveClone generated LiveCD
            ...
            minimum_free_size = 500 * 1024 # 50 M
            size_of_installation = getDirectorySize("/mnt/salt/mnt/modules/") + minimum_free_size
            percent=0.15
            lock = threading.Lock()
            mthread = threading.Thread(target=self.install_modules, args=(self, Main_MountPoint, modules, lock))
            mthread.start()
            while mthread.is_alive():
		space_used=float(commands.getoutput("df | grep %s | awk '{print $3}'" % Selected_Main_Partition))
            ...
            #thread.start_new_thread(self.install_modules, (self, Main_MountPoint, modules, lock,size_of_installation))
            #percent=0.15
            #while percent < 0.3:
		...
		space_used=float(commands.getoutput("df | grep %s | awk '{print $3}'" % Selected_Main_Partition))
                ...
                yield True
                time.sleep(2)
		#self.InstallProgressBar.set_text(str(stop_progress_bar))
                #space_used=float(commands.getoutput("df | grep %s | awk '{print $3}'" % Selected_Main_Partition))
                #percent = 0.15+((space_used/size_of_installation)*0.50)
                #percent = round(percent,2)
                #if percent > self.InstallProgressBar.get_fraction():
        ...
        if LinMountSets != [] :
            for i in LinMountSets : # Here, i[1] value will be 'Select...'
                ...
                blkid_fstype_output = "blkid | grep -A50 " + i[0] + " | grep -m 1 'TYPE=' "
                Lin_Filesys_String = commands.getoutput(blkid_fstype_output)
...
use the application of your choice before rebooting your machine.)\n"""))
    ...
    def chroot_settings(self) :
        ...
        if os.path.exists('/usr/sbin/update-all'):
            subprocess.call('/usr/sbin/update-all', shell=True)
...
def getDirectorySize(directory):
    ...
    """
    return int(commands.getoutput("du -s -l '" + directory + "' | awk '{print $1}' "))
